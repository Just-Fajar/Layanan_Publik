    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm max-w-4xl mx-auto">
            <form method="GET" action="{{ route('calendar.index') }}" class="flex flex-col md:flex-row gap-3">
                ...
                <!-- Submit Button -->
                <button type="submit" class="px-6 py-3 bg-brand-700 hover:bg-brand-800 text-white font-heading font-bold text-sm rounded-xl transition shadow-sm flex items-center justify-center gap-2">
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4 max-w-4xl mx-auto mt-6 pt-4 border-t border-slate-200">
            <div class="flex items-center gap-2 overflow-x-auto w-full sm:w-auto py-1">
                ...
                @foreach(array_slice($categories, 0, 4) as $catKey => $catLabel)
                    <a href="{{ route('calendar.index', ['category' => $catKey]) }}" class="px-3.5 py-1.5 rounded-full text-xs font-heading font-semibold transition whitespace-nowrap {{ request('category') == $catKey ? 'bg-brand-700 text-white shadow-sm' : 'bg-white border border-slate-200 text-slate-600 hover:border-slate-300' }}">
                        {{ $catLabel }}
                    </a>
                @endforeach
            </div>
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <h2 class="text-xl sm:text-2xl font-heading font-bold text-slate-900 mb-8 flex items-center gap-2">
            ...
            @foreach($events as $event)
                <article class="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-xl hover:border-brand-300 transition-all duration-300 flex flex-col group">
                    ...
                    <div class="relative aspect-video bg-slate-100 overflow-hidden">
                        @if($event->image_url)
                            <img src="{{ $event->image_url }}" alt="{{ $event->title }}" width="400" height="225" loading="lazy" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                        @else
                            <div class="w-full h-full flex items-center justify-center bg-gradient-to-br from-brand-700 to-indigo-800 text-white">
                        ...
                        @if($event->start_date)
                            <div class="absolute top-3 left-3 bg-white/95 backdrop-blur-md rounded-xl px-3 py-1.5 shadow-md border border-white/60 text-center leading-none">
                                ...
                                <span class="block text-[10px] font-heading font-bold uppercase text-brand-700 mt-0.5">{{ $event->start_date->format('M') }}</span>
                            </div>
                        @endif
                    ...
                    <div class="p-6 flex flex-col flex-grow justify-between">
                        <div>
                            ...
                            <div class="flex flex-wrap items-center gap-3 text-xs text-slate-500 mb-3">
                                @if($event->start_date)
                                    <div class="flex items-center gap-1.5">
                                        <i class="fa-regular fa-clock text-brand-700"></i>
                            ...
                            <!-- Title -->
                            <h3 class="font-heading font-bold text-slate-900 text-lg leading-snug mb-2 group-hover:text-brand-700 transition">
                        ...
                        <div class="pt-4 border-t border-slate-100 mt-auto flex items-center justify-between">
                            <span class="text-xs font-heading font-bold {{ $event->is_upcoming ? 'text-emerald-700' : 'text-slate-500' }}">
                                <i class="fa-solid fa-circle text-[8px] mr-1"></i>
                                {{ $event->is_upcoming ? 'Mendatang' : 'Tersedia' }}
                            </span>

                            <a href="{{ route('calendar.show', $event) }}" class="inline-flex items-center gap-1.5 text-xs font-heading font-bold text-brand-700 hover:text-brand-800 group-hover:translate-x-1 transition-transform">
