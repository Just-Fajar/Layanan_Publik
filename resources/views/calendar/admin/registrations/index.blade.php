...
@section('title', 'Registration Management - Calendar Admin')
...
<div class="p-6">
        ...
        <div class="border-b border-gray-200">
            <nav class="flex -mb-px">
                <a href="{{ route('calendar.admin.registrations.index') }}" 
                   class="px-6 py-4 text-sm font-medium {{ !request('status') ? 'border-b-2 border-purple-500 text-purple-600' : 'text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                    All Registrations
                    <span class="ml-2 px-2 py-1 text-xs rounded-full {{ !request('status') ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-600' }}">
                        {{ $total_count }}
                    </span>
                </a>
                <a href="{{ route('calendar.admin.registrations.index', ['status' => 'registered']) }}" 
                ...
                <a href="{{ route('calendar.admin.registrations.index', ['status' => 'attended']) }}" 
                ...
                <a href="{{ route('calendar.admin.registrations.index', ['status' => 'cancelled']) }}" 
    ...
    <div class="bg-white rounded-lg shadow-md p-4 mb-6">
        <form method="GET" action="{{ route('calendar.admin.registrations.index') }}" class="flex gap-4">
            ...
            @if(request('search'))
                <a href="{{ route('calendar.admin.registrations.index', ['status' => request('status')]) }}" class="bg-gray-600 hover:bg-gray-700 text-white px-6 py-2 rounded-lg transition">
                    ...
                    @forelse($registrations as $registration)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="text-sm text-gray-500">{{ $registration->event->start_date ? $registration->event->start_date->format('d M Y, H:i') : '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($registration->qr_code)
                                    <span class="text-green-600 text-sm"><i class="fas fa-check-circle mr-1"></i> Generated</span>
                                @else
                                    <span class="text-gray-400 text-sm"><i class="fas fa-times-circle mr-1"></i> Not Generated</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="{{ route('calendar.admin.registrations.show', $registration->id) }}" class="text-purple-600 hover:text-purple-900">
