...
{
    public function index(){ $rows = News::latest()->paginate(10); return view('esport.admin.news.index', compact('rows')); }
    public function create(){ return view('esport.admin.news.create'); }

    public function store(StoreNewsRequest $r){
        $data = $r->validated();
        if ($r->hasFile('image')) $data['image'] = $r->file('image')->store('news','public');
        News::create($data);
        return redirect()->route('esport.admin.news.index')->with('ok','Created');
    }

    public function edit(News $news){ return view('esport.admin.news.edit', compact('news')); }
